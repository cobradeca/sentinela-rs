...
// Municípios com histórico real de enchentes no RS — fonte: ANA, CPRM, Defesa Civil RS
...
function getRiskLevel(precipAccum, tempMin, windMax, lagoaLevel = null) {
  ...
  if (ENSO.nino34 >= 1.5) score += 3; else if (ENSO.nino34 >= 0.5) score += 2; else if (ENSO.nino34 > 0) score += 1;
  if (lagoaLevel !== null) {
...
// ANA HidroWeb — nível real da lagoa
...
export default function SentinelaRS() {
  ...
  const loadAllData = useCallback(async () => {
    ...
    for (const st of STATIONS) {
      try {
        const weather = await fetchWeather14Days(st.lat, st.lon);
        let realLevel = null;
  ...
  return (
    <div style={{ minHeight:"100vh", background:t.bg, color:t.text, fontFamily:"'IBM Plex Mono','Courier New',monospace", transition:"background 0.3s,color 0.3s" }}>
      ...
      <div style={{ position:"relative", zIndex:2, maxWidth:1200, margin:"0 auto", padding:"0 16px 40px" }}>
          ...
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6 }}>
              ...
              <div style={{ padding:"4px 10px", borderRadius:4, border:`1px solid ${ensoClass.color}55`, color:ensoClass.color, fontSize:10, fontWeight:700 }}>
                {ensoClass.icon} El Niño em Desenvolvimento · +{ENSO.nino34}°C · {(ENSO.prob.elNino*100).toFixed(0)}% prob.
              </div>
        ...
        {/* ══ LAGOA DOS PATOS — nível real ANA em todos os pontos ══ */}
        {!loading && activeTab==="lagoa" && (
          <div style={{ display:"grid", gap:10 }}>
            <div style={{ fontSize:10, color:t.textMuted, marginBottom:4 }}>
              4 pontos de monitoramento · ANA HidroWeb (telemetria horária) · El Niño eleva risco costeiro em Rio Grande
            </div>
            {STATIONS_LAGOA.map(station => {
              const d=stationData[station.id];
                ...
                <div key={station.id} style={{ ...s.card }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                    <div>
                      ...
                      <div style={{ fontSize:9, color:t.textMuted }}>
                        ...
                        <span style={{ color:isReal?"#22c55e":t.textMuted, marginLeft:6 }}>{isReal?"● Dado Real ANA HidroWeb":"○ Estimado (ANA indisponível)"}</span>
                      </div>
                    </div>
                    <div style={{ fontSize:10, color:rColor, border:`1px solid ${rColor}`, background:getRiskBg(d.risk), padding:"3px 8px", borderRadius:3 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6 }}>
                    {[["Normal","0.5m","#22c55e"],["Alerta","0.8m","#f97316"],["Emergência","1.2m","#ef4444"]].map(([l,v,c])=>(
                      <div key={l} style={{ background: dark?"rgba(0,0,0,0.3)":t.bg, padding:"6px 8px", borderRadius:3, borderLeft:`3px solid ${c}` }}>
            ...
            <div style={{ padding:"10px 14px", background: dark?"rgba(34,211,238,0.04)":"rgba(8,145,178,0.05)", border:`1px solid ${dark?"rgba(34,211,238,0.15)":t.border}`, borderRadius:5, fontSize:9, color:t.textMuted }}>
              📡 Fonte: ANA HidroWeb — Telemetria (quando disponível) · Estações: 87980000, 87955000, 87540000, 87450004 · Atualização: horária · Estimativas por Open-Meteo quando indisponível
            </div>
          </div>
        )}
          ...
          <div style={{ display:"grid", gap:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:10 }}>
              {[
                ...
                { l:"Limiar Super El Niño",   v:`+${(ENSO.superThreshold-ENSO.nino34).toFixed(1)}°C restam`, s:`Limiar: +${ENSO.superThreshold}°C`, c:"#eab308" },
              ].map(item=>(
                ...
                <div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:8 }}>
                    {[
                      ...
                      <div key={item.l} style={{ background: dark?"rgba(0,0,0,0.3)":t.bg, padding:"8px 10px", borderRadius:3 }}>
                        <div style={{ fontSize:8, color:t.textMuted }}>{item.l}</div>
          ...
          <div>
            ...
            {alerts.length===0 ? (
              <div style={{ textAlign:"center", padding:50, border:"1px solid rgba(34,197,94,0.3)", borderRadius:5, background:"rgba(34,197,94,0.05)" }}>
                ...
                <div style={{ fontSize:10, color:t.textMuted, marginTop:5 }}>Estações dentro dos parâmetros normais.</div>
              </div>
            ) : (
              <div style={{ display:"grid", gap:8 }}>
            ...
            {[
              ...
              <div key={api.n} style={{ background:t.cardBg, border:`1px solid ${t.border}`, borderRadius:5, overflow:"hidden", boxShadow:t.shadowCard }}>
                <div style={{ padding:"12px 14px", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12, fontWeight:600, color:t.text, marginBottom:3 }}>{api.n}</div>
        ...
        <div style={{ marginTop:28, borderTop:`1px solid ${t.border}`, paddingTop:12, display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:6 }}>
          <div style={{ fontSize:9, color:t.textFaint }}>SENTINELA·RS v2.2 · Open-Meteo + ANA HidroWeb + NOAA ENSO + INPE + Copernicus</div>
